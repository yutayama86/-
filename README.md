...
毎日記事を書くのではなく、**その日いちばん期待値の高い1ページへ投資する**処理です。

1. Search Console と GA4 からデータを取得
2. 投資先を1つだけ決める（`scripts/lib/decide-target.mjs`）
3. 決まった対象**そのもの**を改善する
   - 既存ページ → `scripts/improve-page.mjs` が、必要な箇所だけを置換する改善案をAIに作らせて適用
   - 新規記事 → `new_article` が選ばれた日だけ、テーマ選定と記事生成を行う
   - 計測改善 → コンテンツは触らず、レポートだけ残す
4. `docs/seo-log/` に判断と根拠を記録
5. テスト・型チェック・ビルド・品質チェック・SEO監査・**分析対象と変更ファイルの一致確認**（`npm run verify`）
6. Pull Request を作成

**マージするまで公開されません。** 5で落ちた場合は変更を破棄し、レポートだけを残してActionsを失敗として表示します。分析した対象と別のページが変更された場合もPRは作られません。判断は `.growth/target.json` に書き出され、後続のスクリプトがそれを読んで動くため、ワークフローの書き方に依存しません。

設定済みのGA4またはSearch Consoleからデータを取得できない日は、誤った判断を避けるためコンテンツを変更しません。
