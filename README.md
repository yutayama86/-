...
5. **分析対象と変更ファイルの一致を確認**（`scripts/verify-target-match.mjs`）
6. テスト・型チェック・ビルド・品質チェック・SEO監査
7. Pull Request を作成

**マージするまで公開されません。** 5か6で落ちた場合は変更を破棄し、レポートだけを残してActionsを失敗として表示します。分析した対象と別のページが変更された場合もPRは作られません。
