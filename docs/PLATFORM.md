...
`places`（4件）、`stores`（12件）、`businesses.ts`（4件）が並立している。

- `places` と `stores` は**全件 draft**。中身は実装確認用のサンプル名
  （「珈琲 ひとひら」「サウナ 森の音」など）で、**実在しない**。公開してはいけない。
- `businesses.ts` は**実在を確認した事業者だけ**。イバトコはページを持たず、
...
- **`places` / `stores` が全件 draft**。サンプル名が入っているため。
  実在の店に差し替えるまで公開しない
- **`commercialPriority` が未設定**。GSCのデータを見てから入れる（投資判断なので推測しない）
