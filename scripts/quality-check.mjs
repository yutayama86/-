...
import { findLegacyTerms } from './lib/brand-guard.mjs';
...
function check(file,raw){const p=parse(raw);if(!p){err(file,'frontmatterを解析できません');return}const{data,body}=p;if(file.includes('/knowledge/')){if(!data.intent)err(file,'intent（検索意図）が未設定です');if(!data.primaryKeyword)err(file,'primaryKeyword が未設定です');const links=body.match(/\]\(\/(knowledge|service|case|contact)\/?/g)??[];if(!links.length)err(file,'関連記事・支援・事例・相談への内部導線がありません');if(!/^##\s*(まとめ|結論)/m.test(body))warn(file,'まとめ/結論の見出しがありません');const sections=[...body.matchAll(/^(#{2,3})\s+(.+)$/gm)];for(const[i,m]of sections.entries()){const head=m[2].trim();if(head.length>60)err(file,`見出しが長すぎます（${head.length}文字）: ${head.slice(0,40)}…`);const nextSection=sections[i+1];const next=nextSection?.index??body.length;const inner=body.slice(m.index+m[0].length,next).trim();const hasSubsection=nextSection&&nextSection[1].length>m[1].length;if(!inner&&!hasSubsection)err(file,`中身のない見出しがあります: ${head.slice(0,40)}`)}}
const broken=body.match(/[】）〕](\((?:\/|https?:)[^)]*\))/);if(broken)err(file,`Markdownのリンク記法が壊れています: ${broken[0].slice(0,40)}`);
const banned=['株式会社シクミベース','代表取締役','弊社は法人'];for(const w of banned)if(raw.includes(w))err(file,`法人化前のため使用できない表記: ${w}`);
const legacy=['リフォーム反響OS 30','見積フォロー漏れ診断','住宅リフォーム会社のあなたは','リフォーム反響OS','/service/reform-lead-os/','/diagnosis/reform-lead/'];for(const w of legacy)if(raw.includes(w))err(file,`旧リフォーム特化ブランド表現が残っています: ${w}`);
...
if(String(data.generated??'').replace(/["']/g,'')==='true'){const legacyGenerated=findLegacyTerms(raw);if(legacyGenerated.length)err(file,`自動生成記事に旧リフォーム特化文脈が含まれています: ${legacyGenerated.join('、')}`);if(chars<1800)err(file,`自動生成記事が短すぎます（約${chars}文字）`);if(!/(仕組|標準|再現|属人)/.test(body))err(file,'自動生成記事がシクミベースの「再現可能な仕組み」という中核テーマに接続していません');if(!/(人が確認|人間が確認|承認|レビュー)/.test(body)&&/(AI|自動化)/.test(body))warn(file,'AI/自動化記事に人間確認・承認の境界が明示されていません')}
}
